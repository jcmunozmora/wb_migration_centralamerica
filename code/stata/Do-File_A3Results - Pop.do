...
***Información a nivel departamental***
	...
	* Merge with pop 
	...
	gen var_1000 = (valor/poblacion) * 1000
	drop _merge
	gen valor2 = valor
	replace valor2 = var_1000 if strpos(id_indicador, "ACLEC") == 1
	drop valor 
	rename valor2 valor
	replace valor = 0 if valor == .
...
save "variables_general_A3.dta", replace 
restore	
...
save "Depto_priorizado_foodsecurity_A3.dta", replace 
restore
...
drop if id_indicador == "ACLEC _3_3_1" | id_indicador == "ACLEC _3_3_2" | id_indicador == "ACLEC _3_3_3" | id_indicador == "ACLEC _3_3_4" | id_indicador == "ACLEC _3_3_5" | id_indicador == "ACLEC _3_3_6" | id_indicador == "ACLEC _3_3_7" | id_indicador == "ACLEC _3_3_8" | id_indicador == "ACLEC _3_3_5_tot" | id_indicador == "ACLEC _3_3_6_tot" | id_indicador == "ACLEC _3_3_7_tot" | id_indicador == "ACLEC _3_3_8_tot"
   ...
   gen puntaje_minmax_`a' = 1 - ((r(max) - puntaje) / (r(max) - r(min)))
    ...
    save "priorizacion_mainA3`a'.dta", replace
    restore
}
...
use  "priorizacion_mainFragility.dta"
mmerge pais departamento using "priorizacion_mainA3Violence.dta"
...
save "temporalA3.dta", replace
restore



preserve
use "variables_general_A3.dta", clear
keep pais departamento indicador valor normalizado
replace pais = "EL SALVADOR" if pais == "SALVADOR" 
mmerge pais departamento using "temporalA3.dta"
mmerge pais departamento using "C:/Users/user/OneDrive - Universidad EAFIT/2025_WB_LatinAmerica/02_Sampling/Databases/data_orig/Landscan/poblacion.dta", ukeep(poblacion)
...
save "variables_general_categoria_A3.dta",replace
export excel "variables_general_categoria_A3.xlsx", firstrow(variable) replace
restore
...
mmerge pais departamento using "Depto_priorizado_foodsecurity_A3.dta", ukeep(normalizado valor)
rename (normalizado valor) (food_normalizad food_valor)
drop if (departamento == "ZELAYA CENTRAL" | departamento == "BAY ISLANDS" | departamento == "DISTRITO CENTRAL")
mmerge pais departamento using "C:/Users/user/OneDrive - Universidad EAFIT/2025_WB_LatinAmerica/02_Sampling/Databases/data_orig/Landscan/poblacion.dta", ukeep(poblacion)


export excel "variables_A3.xls", firstrow(variable) replace
...
preserve
collapse (count) normalizado (sum) poblacion (mean) puntaje_minmax_Fragility_mean  = puntaje_minmax_Fragility (sd) puntaje_minmax_Fragility_sd  = puntaje_minmax_Fragility  (mean) puntaje_minmax_Violence_mean  = puntaje_minmax_Violence (sd) puntaje_minmax_Violence_sd  = puntaje_minmax_Violence (mean) valor_mean = valor (sd) valor_sd = valor , by (cat_4) 
rename normalizado numero
format puntaje* valor* %9.2f  
export excel "resumen_4A3.xls", firstrow(variable) replace
restore

preserve
collapse (count) normalizado (sum) poblacion (mean) puntaje_minmax_Fragility_mean  = puntaje_minmax_Fragility (sd) puntaje_minmax_Fragility_sd  = puntaje_minmax_Fragility  (mean) puntaje_minmax_Violence_mean  = puntaje_minmax_Violence (sd) puntaje_minmax_Violence_sd  = puntaje_minmax_Violence (mean) valor_mean = valor (sd) valor_sd = valor , by (cat_3) 
rename normalizado numero
format puntaje* valor* %9.2f
export excel "resumen_3A3.xls", firstrow(variable) replace
restore

* Mediana
preserve
collapse (count) normalizado (mean) valor (sum) poblacion (mean) puntaje_minmax_Fragility_mean  = puntaje_minmax_Fragility (sd) puntaje_minmax_Fragility_sd  = puntaje_minmax_Fragility  (mean) puntaje_minmax_Violence_mean  = puntaje_minmax_Violence (sd) puntaje_minmax_Violence_sd  = puntaje_minmax_Violence (mean) valor_mean = valor (sd) valor_sd = valor , by (cat_2) 
rename normalizado numero
format puntaje* valor* %9.2f
export excel "resumen_2A3.xls", firstrow(variable) replace
restore	
